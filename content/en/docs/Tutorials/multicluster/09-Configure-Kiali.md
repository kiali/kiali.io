...
helm upgrade --install --namespace istio-system --set auth.strategy=anonymous --set server.observability.logger.log_level=debug --set deployment.ingress.enabled=true --repo https://kiali.org/helm-charts kiali-server kiali-server
```
