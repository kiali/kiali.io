...
- `ALLOW_AD_HOC_INIT_CONTAINERS`: must be `true` or `false`. If `true`, the operator will be allowed to install additional init containers into the Kiali pod as defined in the Kiali CR's `spec.deployment.additional_pod_init_containers_yaml`. If `false`, the operator will not allow any additional init containers to be configured. If a Kiali CR is created with `spec.deployment.additional_pod_init_containers_yaml` defined, the operator will abort the Kiali Server installation.
