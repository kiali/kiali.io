...
Before talking about Kiali features, it is important to understand that Kiali's performance is dependent on the performance and responsiveness of your metrics database (typically Prometheus), and your tracing store, for installations using tracing. For Prometheus scalability tips, see [Prometheus Tuning]({{< ref "/docs/Configuration/external-services/metrics/prometheus#prometheus-tuning" >}}). See [Tempo Tuning]({{< ref "/docs/Configuration/external-services/tracing/tempo#tempo-tuning" >}}) if using Tempo for your trace store.
