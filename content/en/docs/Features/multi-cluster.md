...
A single Kiali install can work with one metric store and one trace store and can be configured for "single cluster" or "multi-cluster". For multi-cluster deployments, all clusters must be part of the same mesh and report to the same metric and trace store, whether directly or via some sort of aggregator. Kiali can also discover and visualize [multiple meshes within a single cluster](#multiple-meshes-in-a-single-cluster). See the [multi-cluster configuration page]({{< relref "../Configuration/multi-cluster" >}}) for more information on requirements.
