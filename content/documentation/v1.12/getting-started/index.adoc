...
For more, see the link:https://docs.okd.io/3.11/install/prerequisites.html[OKD prerequisites].
...
It is not usually necessary to explicitly execute the link:https://github.com/kiali/kiali/blob/v1.12/operator/deploy/deploy-kiali-operator.sh[Kiali Operator install script] but it does offer a variety of options. To learn more about the script, and the available options, run the command:
...
To create an initial Kiali CR file it is recommended to copy the fully documented link:https://github.com/kiali/kiali/blob/v1.12/operator/deploy/kiali/kiali_cr.yaml[example Kiali CR YAML file]. Edit the file, being careful to maintain proper formatting, and save it to a local file such as `my-kiali-cr.yaml`.
...
Once Istio, Maistra or the Kiali Operator has installed Kiali, and the Kiali pod has successfully started, you can access the UI. Please, check the link:{{< ref "/documentation/v1.12/faq#how-do-i-access-kiai" >}}[FAQ: How do I access Kiali UI?]
...
For further information on how the `label_selector` interacts with `deployment.accessible_namespaces` read the https://github.com/kiali/kiali/blob/v1.12/operator/deploy/kiali/kiali_cr.yaml[technical documentation].
