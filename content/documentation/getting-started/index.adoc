...
All of the installation options below:

* Ask to delete an existing Kiali Operator, or Kiali installation, when appropriate.
* Prompt for the authentication strategy to use and/or additional information needed for login.
** See link:#_login_options[Login Options] for more on the `login`, `anonymous` and `openshift` authentication strategies.


==== Default Installation

This option installs the Kiali Operator and the Kiali CR (Custom Resource), with default options.  Kiali will be able to access existing namespaces, but will require a CR update to access future namespaces (for more, see link:#_namespace_management[Namespace Management]. Run the install script via this command:
...
Like the Default Installation but Kiali will be able to access existing namespaces and future namespaces. Because this grants cluster roles it is not recommended for production. Run the install script via this command:
...
[source,bash]
----
./operator/deploy/deploy-kiali-operator.sh --help
----
...
[source,bash]
----
bash <(curl -L https://git.io/getLatestKialiOperator) --uninstall-mode
----
