...
* ldap_attributes: These are the attributes that the search request pulls from the LDAP server. For example ['memberOf', 'cn', 'mail'].
...
This query says, from the com Domain Component, find the google Domain Component, and then inside it the gl Domain Component and then inside it the gp Domain Component. In the gp Domain Component, find the Organizational Unit called Distribution Groups and then find the the object that has a common name of Dev-India.
