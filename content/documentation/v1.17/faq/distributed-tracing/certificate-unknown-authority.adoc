...
However there is a workaround for that. Kiali can be configured to skip the authority verification through the flag: insecure_skip_verify. Change its value to true in the link:https://github.com/kiali/kiali/blob/v1.16/operator/deploy/kiali/kiali_cr.yaml#L262[Kiali CR,window="_blank"].
