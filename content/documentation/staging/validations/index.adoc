...
=== KIA1109 - When gateway is not set, the default gateway (mesh) is used, which applies the rule to all sidecars in the mesh

A virtual service might not have any gateway set. In this case the default gateway (mesh) might be used, and the rule would be applied to all sidecars in the mesh. Kiali shows info message on these virtuel services.
