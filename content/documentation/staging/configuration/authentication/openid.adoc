...
<<configure-scopes,configure the `email` scope to be requested>>.
...
link:https://developers.google.com/identity/protocols/oauth2/openid-connect#an-id-tokens-payload[includes
this `hd` claim for hosted domains].
...
* Go to _AAD > App Registration_, create an application with a redirect url like `\https://<your-kiali-url>`
...
** After creating the client secret, take note of the provided secret. Create a
   ...
   link:https://github.com/kiali/kiali-operator/blob/5e364ee48c08a1bdd200172b47f08b2ed0369c25/deploy/kiali/kiali_cr.yaml#L342-L347[comments
   for the `secret_name` configuration in the sample Kiali CR].
...
* Go to _Authentication_ and make sure that the _Access tokens_ checkbox is ticked.

image::/images/documentation/authentication/azure-access-token-ticked.png[Access tokens enabled]
