...
The *control* workload has now an Istio sidecar proxy injected but this application is not accessible from the outside.
...
NOTE: *[1]* Create a DNS entry for the *control* service associated with the External IP of the Istio Ingress

icon:bullhorn[size=1x]{nbsp} This step depends on the platform you have choose, please review link:https://istio.io/latest/docs/setup/getting-started/#determining-the-ingress-ip-and-ports[Determining the Ingress IP and Ports] for more details.
...
And we will create a simple entry in the `/etc/hosts` of the tutorial machine with the desired DNS entry:
...
NOTE: *[2]* Use the Request Routing Wizard on *control* service to generate a traffic rule
++++
...
{nbsp} +
Any request will be routed to the *control* workload.
++++
...
NOTE: *[3]* Test the *control* service
++++
...
NOTE: *[4]* Review *travel-control* namespace in Kiali
++++
...
. Extenal traffic enters into the cluster through a Gateway
. Traffic is routed to *control* service through a VirtualService
. Kiali Graph visualizes the telemetry reported from *control* sidecar proxy
.. Only *travel-control* namespace is part of the mesh

link:https://istio.io/latest/docs/reference/config/networking/gateway/[(i) Istio Gateway]
