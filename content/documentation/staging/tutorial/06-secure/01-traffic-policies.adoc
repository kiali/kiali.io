...
link:https://istio.io/latest/docs/concepts/security/[Security] is one of the main pillar of Istio features.
...
NOTE: *[1]* Undeploy the *loadbtester* workload from *travel-portal* namespace
...
We should validate that telemetry has update the *travel-portal* namespace and "Security" can be enabled in the Graph Display options.
...
NOTE: *[2]* Create Authorization Policies for current traffic for *travel-agency* namespace

Every workload in the cluster users a link:https://kubernetes.io/docs/tasks/configure-pod-container/configure-service-account/[Service Account].
...
NOTE: *[3]* Deploy the *loadtester* portal in the *travel-portal* namespace
...
NOTE: *[4]* Update *travels-v1* AuthorizationPolicy to allow *loadtester* ServiceAccount
