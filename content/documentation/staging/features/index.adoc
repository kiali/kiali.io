...
A basic Istio mesh deployment has a single control plane with a single data plane, deployed on a single Kubernetes cluster. But Istio supports a variety of advanced https://istio.io/latest/docs/ops/deployment/deployment-models/[deployment models, window="_blank"]. It allows a mesh to span multiple primary (control plane) and/or remote (data plane only) clusters, and can use a single or https://istio.io/latest/docs/ops/deployment/deployment-models/#multiple-
orks[multi-network, window="_blank"] approach.  The only strict rule is that within a mesh service names are unique. A non-basic mesh deployment generally involves multiple clusters.  See https://istio.io/docs/setup/install/multicluster/[installation instructions, window="_blank"] for more detail on installing advanced mesh deployments.
