...
aliases:
- /features/
- /documentation/staging/overview/
...
Check out the link:/documentation/staging/runtimes-monitoring/[documentation page, window=_blank] to learn more about it.
++++
<div style="display: flex;">
    <span style="margin: 0 auto;">
...
Check the complete link:/documentation/staging/validations/[list of validations] for further information.
...
A *Workload* has also enabled specific actions
...
Also, actions are availble for an entire *Namespace*
...
* Every rule is composed by a Request Matching and a Routes To section.
* The Request Matching section can add multiple filters using HEADERS, URI, SCHEME, METHOD or AUTHORITY Http parameters.
* The Request Matching section can be empty, on this case, any http request received is matched against this rule.
* The Routes To section can select the percentage of traffic that is routed to a specific workload.
...
Istio applies routing rules in order, meaning that first rule that matches an HTTP request, it is responsible to perform the routing. The Matching Routing Wizard allows changing order of rules.
...
The Request Timeouts Wizard permits to set up request timetous in Envoy using Istio.

* HTTP Timeout defines the timeout for a request.
* HTTP Retry describes the retry policy to use when a HTTP request fails.
...
One available Advanced Option is to expose a Service to external traffic through an existing Gateway or create a new Gateway for this Service.
...
* Automatically generated a PeerAuthentication resource for this Service.
...
A default scenario is to indicate this at *Namespace* level but there can be cases where a *Workload* shouldn't be part of the Mesh or viceversa.

Kiali allows to annotate the Deployment template and propagate this configuration into the Pods.
...
Kiali offers several actions for a *Namespace* located in the Overview page in any of its views: Expanded, Compacted or Table.
...
Show actions allows to navigate from a *Namespace* to its specific Graph, Applications, Workload, Services or Istio Config pages.

==== Automatic Sidecar Injection

When https://istio.io/latest/docs/setup/additional-setup/sidecar-injection[Automatic Sidecar Injection] is enabled in the cluster, a *Namespace* can be labeled to enable/disable the injection webhook in the namespace, then new deployments will automatically have a sidecar added to them.

==== Security: Traffic Policies

Kiali can generate Traffic Policies based on traffic for a namespace.
...
And a user may want to add Traffic Policies to secure that communication, in other words, to not allow other traffid different to the represented by the Services and Workloads in the Graph.

Using *Create Traffic Policies* action on a namespace Kiali will generate AuthorizationPolicy resources per every *Workload* in the *Namespace*.
...
Kiali establishes reasonable defaults for determining the health of request traffic. But when the defaults are not appropriate Kiali offers a way to customize the thresholding used to determine degraded or failing request traffic for resources (service, workload, app...). See the link:/documentation/latest/configuration/health/[Health Configuration documentation] for more detail.

++++
<div style="display: flex;">
 <span style="margin: 0 auto">
  <a class="image-popup-fit-height" href="/images/documentation/health-configuration/beta.png" title="beta Namespace">
   <img src="/images/documentation/health-configuration/beta.png" style="width: 300px;height:400px;display:inline;margin: 0 auto;" />
  </a>
  <a class="image-popup-fit-height" href="/images/documentation/health-configuration/details_health.png" title="Health Deatails">
   <img src="/images/documentation/health-configuration/details_health.png" style="width: 300px;height:400px; display:inline;margin: 0 auto;" />
  </a>
 </span>
</div>
++++
