...
The Istio service mesh architecture is based in different components: from the istiod to Jaeger. Those components are key for the mesh to work well. That is why Kiali runs regular checks looking for the status of these Istio components.
...
There are two possible *statuses* which a component may be in: `Not healthy` or `Not found`. The `Not healthy` status means that the deployment doesn't have the desired amount of replicas running. The `Not found` status means that Kiali is not able to find the deployment.

Regarding the *severity* of each component, there is only to options: `core` or `add-on`. The `core` components are those shown as errors (in red) whereas the `add-ons` are displayed as warnings (in orange).

By default, Kiali checks the following components installed in the control plane namespace: istiod, ingress, prometheus (as core components), egress, grafana and jaeger (as add-ons).
