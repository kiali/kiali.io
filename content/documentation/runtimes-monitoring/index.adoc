...
Additionally, you must tell Kiali the URL of that Prometheus server either by setting the environment variable `PROMETHEUS_CUSTOM_METRICS_URL` or by modifying the Kiali _ConfigMap_ or _Kiali CR_.
