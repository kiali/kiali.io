...
icon:bullhorn[size=1x]{nbsp} If you had previously installed the `demo` profile but without `jaegerURL` or `grafanaURL` variables, they can still be defined. Modify the `kiali` configmap found in your control plane namespace. See how: link:{{< ref "/installation-guide#_create_or_edit_the_kiali_cr" >}}[here].
...
icon:bullhorn[size=1x]{nbsp} The default credentials to log in are: `admin/admin`. It is highly recommended to change them as soon as possible, see the link:{{< ref "/installation-guide#_login_options" >}}[Login Options]
