...
Kiali didn't find the tracing service. Kiali tries to search for the tracing service in `tracing` or `jaeger-query`. If you have another name for this service you must set the values in the link:https://github.com/kiali/kiali-operator/blob/master/deploy/kiali/kiali_cr.yaml[yaml configuration, window="_blank"] when installed with link:https://operatorhub.io/operator/kiali[Kiali Operator, window="_blank"]
