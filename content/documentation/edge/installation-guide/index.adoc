...
icon:bullhorn[size=1x]{nbsp} Starting with Istio 1.7, Kiali is no longer shipped with the istioctl installer. If you are using an  Istio implementation of version 1.7 or higher, this section is not valid for you.
...
The next few sections describe the different installation methods available for installing the Kiali Operator. Depending on the installation method you use, the operator may then install Kiali. Before you install Kiali, you will want to know what authentication strategy Kiali should use (this determines how users are to log into Kiali). See link:#_login_options[Login Options] for more on the `login`, `anonymous`, `ldap`, `openshift`, `token` and `openid` authentication strategies.
...
In this case, Kiali can access currently existing namespaces as well as future namespaces that may be created later. See link:#_namespace_management"[Namespace Management] for more information if you want to change that behavior.
...
For production environments that do not have OperatorHub, you will most likely want to use link:https://kiali.org/kiali-operator/charts/index.yaml[the Helm Chart] rather than the install script. In fact, the install script uses the Helm Chart to perform the installation.
