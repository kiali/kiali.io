...
This option installs the Kiali Operator and the Kiali CR with default options. If you are running with an https://github.com/istio/istio/releases[upstream Istio release], you will probably want to pass in _--accessible_namespaces="**"_ since that is the typical way Kiali is expected to run within an upstream Istio environment.
