...
Selector-less sidecars are those sidecars that doesn't have the `workloadSelector` field and thus they are pointing to all workloads running in the same namespace.
...
In the host field, there is the list of hosts exposed to the workload. Each host in the list must have the `namespace/dnsName` format.
