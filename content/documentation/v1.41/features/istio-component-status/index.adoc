...
If you add additional secrets, the Kiali operator _also_ needs the same privileges in order to configure Kiali successfully. If you used the link:{{< relref "../installation-guide/install-with-helm" >}}[Helm charts] to install the operator, specify the `secretReader` value with the required secrets:
...
If you installed the operator via the link:{{< relref "../installation-guide/installing-with-operatorhub" >}}[OperatorHub] you need to update the operator privileges as a post-installation step, as follows:
